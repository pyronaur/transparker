...
function parsePortFromConfig(value: unknown, configFile: string): number | undefined {
  ...
  if (parsed === undefined || parsed < 1 || parsed > 65535) {
    throw new Error(`Invalid port in Transparker config at ${configFile}: "${String(value)}"`);
  }
...
function parseConfigFile(path: string): RawConfig {
  ...
  try {
    ...
    throw new Error(`Failed reading Transparker config at ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid TOML in Transparker config at ${path}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export function loadTransparkerFileDefaults(
  ...
  const wordlistPath = expandPath(asString(rawConfig.wordlist_file) ?? defaultWordlistPath, configDir);
  const promptPath = expandPath(asString(rawCodex.prompt_file) ?? defaultPromptPath, configDir);

  const seededWordlist = readSeedFile(resolve(projectRoot, "WORDLIST.md"), DEFAULT_WORDLIST_FALLBACK);
